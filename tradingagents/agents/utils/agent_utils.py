...
from tradingagents.dataflows.market_data_utils import get_market_turnover, get_sector_fund_flow,get_market_score,get_market_distribution,get_SSE_datas,get_favored_sectors
...
"""
    ...
    def get_market_turnover() -> str:
        ...
        同花顺API获取市场成交额分时数据
        Returns:
            str: 包含时间、当日成交额、昨日成交额及变动的格式化数据
        """
        try:
            df = get_market_turnover()
            return f"市场成交额分时数据:\n{df.to_markdown()}" if not df.empty else "未获取到成交额数据"
        except Exception as e:
            return f"获取成交额数据失败: {str(e)}"
    ...
    def get_market_distribution() -> str:
        ...
        同花顺API获取市场涨跌分布数据
        Returns:
            str: 包含涨跌家数、涨停跌停数量及详细分布的格式化报告
        """
        try:
            result = get_market_distribution()
            if result and result.status_code == 0:
                data = result.data
                return f"""市场涨跌分布:
    - 涨停: {data.limit_up} 家
    - 跌停: {data.limit_down} 家
    - 上涨: {data.up} 家
    - 下跌: {data.down} 家
    - 平盘: {data.flat} 家
    - 停牌: {data.suspend} 家
    - 最后更新: {data.last_update_time}
    详细分布: {[item.key + ':' + str(item.value) for item in data.table]}"""
            return "未获取到涨跌分布数据"
        except Exception as e:
            return f"获取涨跌分布数据失败: {str(e)}"

    @staticmethod
    @tool
    def get_market_score(date: Annotated[str, "日期，格式示例：20250403"]) -> str:
        """
        同花顺API获取市场综合评分
        Args:
            date: 日期字符串，格式为YYYYMMDD
        Returns:
            str: 包含综合评分、财务评分、技术评分的报告
        """
        try:
            score = get_market_score(date)
            if score:
                return f"""市场综合评分:
    - 标题: {score['score_title']}
    - 综合得分: {score['sum_socre']}
    - 财务评分: {score['finance_score']}
    - 技术评分: {score['tech_score']}
    - 评价: {score['score_content']}"""
            return "未获取到市场评分数据"
        except Exception as e:
            return f"获取市场评分失败: {str(e)}"
    ...
    def get_favored_sectors() -> str:
        ...
        """
        try:
            return get_favored_sectors()
        except Exception as e:
            return f"获取热门板块失败: {str(e)}"
    ...
    def get_sector_fund_flow(sector_type: Annotated[str, "板块类型，如'行业'、'概念'、'地域'"]) -> str:
        ...
        AkShare获取板块资金流排名
        Args:
            sector_type: 板块类型，可选值包括'行业'、'概念'、'地域'
        Returns:
            str: 包含涨跌幅排名前10与后10的板块数据
        """
        try:
            return get_sector_fund_flow(sector_type)
        except Exception as e:
    ...
    def get_SSE_datas() -> str:
        ...
        try:
            data = get_SSE_datas()
            return f"{data[0]}: 点位 {data[1]}, 涨跌幅 {data[2]}, 涨跌额 {data[3]}"
        except Exception as e:
